...
async function showPracticeV2(id){
  ...
  function stopLoop(){state.looping=false;if(loopTimer){clearTimeout(loopTimer);loopTimer=null}}
  function lineEnd(){const next=state.song.lines[state.index+1];if(next)return next.startSeconds+Number(state.prefs.tailSeconds||0);if(Number.isFinite(audio.duration))return audio.duration;return state.song.lines[state.index].startSeconds+6}
  async function playCurrent(){const line=state.song.lines[state.index];audio.currentTime=Math.max(0,line.startSeconds-Number(state.prefs.leadSeconds||0));audio.playbackRate=Number(state.prefs.speed||1);try{await audio.play()}catch(error){console.warn(error)}}
  ...
  audio.addEventListener('timeupdate',()=>{if(audio.currentTime>=lineEnd()){audio.pause();const line=state.song.lines[state.index];api('/api/lyrics/'+line.id+'/practice',{method:'POST'}).catch(()=>{});if(state.looping)loopTimer=setTimeout(playCurrent,Number(state.prefs.gapSeconds||0)*1000)}});
