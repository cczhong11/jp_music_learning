...
async function showPracticeV2(id){
  ...
  audio.addEventListener('timeupdate',()=>{if(activePlayback.lineIndexes.length>1){let visibleIndex=activePlayback.lineIndexes[0];for(const index of activePlayback.lineIndexes){if(audio.currentTime>=state.song.lines[index].startSeconds)visibleIndex=index;else break}if(visibleIndex!==state.index){state.index=visibleIndex;if(currentView==='practice')renderPractice()}}if(audio.currentTime>=activePlayback.end){audio.pause();activePlayback.lineIndexes.forEach(index=>{const line=state.song.lines[index];api('/api/lyrics/'+line.id+'/practice',{method:'POST'}).catch(()=>{})});if(state.looping){const mode=state.loopMode||'sentence';loopTimer=setTimeout(()=>mode==='range'?playRange(true):playCurrent(true),Number(state.prefs.gapSeconds||0)*1000)}}});
